...
def start_analysis(mode, input_source):
    ...
    try:
        if mp_hands and mp_face_mesh:
            hands_ctx = mp_hands.Hands(min_detection_confidence=0.7, min_tracking_confidence=0.7)
            face_ctx = mp_face_mesh.FaceMesh(min_detection_confidence=0.7, min_tracking_confidence=0.7)
        else:
            hands_ctx = None
            face_ctx = None
            ...
            while cap.isOpened():
                ...
                analyze_hands(frame, hands_ctx)

                # Analyze eye direction and head movement
                analyze_face(frame, face_ctx)
...
if __name__ == "__main__" and os.environ.get("SPEECH_UI", "0") == "1":
    show_mode_selector()
